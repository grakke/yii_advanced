...
class MainController extends Controller
{
	...
	public function actionIndex()
	{
		...
		if ($this->checkMainProcess()) {
			echo "Main process is running. Please check it.";
			exit;
		}
		//每N秒检查一次任务列表
		while (1) {
		...
		foreach ($list as $task) {
			...
			switch ($task->type) {
				...
				case ConsoleTask::TYPE_EVERYDAY_FIX_TIME:
					...
					if ($now >= strtotime($task->start_time) && !(date("Y-m-d",
								strtotime($task->last_start_time)) == date("Y-m-d") && $task->status == ConsoleTask::STATUS_FINISHED)) {
						//每天超过指定时间便执行
	...
	public function checkStartedTasks()
	{
		...
		$list = ConsoleTask::findAll([
			"status" => ConsoleTask::STATUS_STARTED, "is_deleted" => ConsoleTask::DELETED_NO
		]);
		$now = time();
		foreach ($list as $task) {
